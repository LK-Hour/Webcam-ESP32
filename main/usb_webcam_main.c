...
static void camera_boot_test(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Boot camera test: init failed: %s", esp_err_to_name(err));
        return;
    }

    sensor_t *sensor = esp_camera_sensor_get();
    ESP_LOGI(TAG, "Boot camera test: sensor PID=0x%04x", sensor->id.PID);
    camera_fb_t *frame = esp_camera_fb_get();
    if (frame == NULL) {
        ESP_LOGE(TAG, "Boot camera test: JPEG capture failed");
    } else {
        bool jpeg_valid = frame->format == PIXFORMAT_JPEG && frame->len >= 4 &&
                          frame->buf[0] == 0xff && frame->buf[1] == 0xd8 &&
                          frame->buf[frame->len - 2] == 0xff && frame->buf[frame->len - 1] == 0xd9;
        ESP_LOGI(TAG, "Boot camera test: %ux%u, %u bytes, JPEG %s",
                 frame->width, frame->height, (unsigned)frame->len,
                 jpeg_valid ? "valid" : "invalid");
